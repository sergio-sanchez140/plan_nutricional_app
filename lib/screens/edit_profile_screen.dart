...
class _EditProfileScreenState extends State<EditProfileScreen> {
  ...
  void initState() {
    ...
    _activity = _safeDropdownValue(u['nivel_actividad'], ["sedentario", "ligero", "moderado", "activo", "muy_activo"]);
  ...
  Future<void> _saveData() async {
    ...
    final newWeight = double.tryParse(_weightCtrl.text.replaceAll(',', '.')) ?? 70.0;
    final newHeight = double.tryParse(_heightCtrl.text.replaceAll(',', '.')) ?? 170.0;
    ...
    try {
      ...
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Error al guardar los datos"), backgroundColor: Colors.red));
    } finally {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text("Datos y Objetivo", style: TextStyle(color: Colors.black, fontSize: 18, fontWeight: FontWeight.bold)),
        backgroundColor: Colors.white,
        elevation: 0,
        iconTheme: const IconThemeData(color: Colors.black),
        actions: [
          _isSaving
              ? const Padding(padding: EdgeInsets.all(16.0), child: SizedBox(width: 20, height: 20, child: CircularProgressIndicator(strokeWidth: 2, color: Colors.green)))
              : TextButton(
                  onPressed: _saveData,
                  child: const Text("Guardar", style: TextStyle(color: Colors.green, fontSize: 16, fontWeight: FontWeight.bold)),
                ),
        ],
      ),
      body: Form(
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              _buildFormCard([
                _buildDropdown("Nivel de Actividad", _activity, [
                  const DropdownMenuItem(value: "sedentario", child: Text("Sedentario")),
                  const DropdownMenuItem(value: "ligero", child: Text("Ligero")),
                  const DropdownMenuItem(value: "moderado", child: Text("Moderado")),
                  const DropdownMenuItem(value: "activo", child: Text("Activo")),
                  const DropdownMenuItem(value: "muy_activo", child: Text("Muy Activo")),
                ], (v) => setState(() => _activity = v)),
                _buildDivider(),
                _buildDropdown("Objetivo", _goal, [
                  const DropdownMenuItem(value: "perder", child: Text("Perder Grasa")),
                  const DropdownMenuItem(value: "mantener", child: Text("Mantener Peso")),
                  const DropdownMenuItem(value: "ganar", child: Text("Ganar Músculo")),
                ], (v) => setState(() => _goal = v)),
              ]),
  ...
  Widget _buildDivider() => const Divider(height: 1, indent: 16, endIndent: 0, color: Color(0xFFEEEEEE));

  Widget _buildTextField(String label, TextEditingController controller, {bool isNumber = false, bool isDecimal = false, bool isRequired = false}) {
    return Padding(
      padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 4),
      child: Row(
        children: [
          SizedBox(width: 100, child: Text(label, style: const TextStyle(fontWeight: FontWeight.w600, fontSize: 15))),
          Expanded(
            child: TextFormField(
              ...
              validator: isRequired ? (v) => v!.isEmpty ? "Obligatorio" : null : null,
            ),
          ),
        ],
      ),
  ...
  Widget _buildDropdown(String label, String? value, List<DropdownMenuItem<String>> items, ValueChanged<String?> onChanged) {
    return Padding(
      ...
      child: Row(
        children: [
          SizedBox(width: 130, child: Text(label, style: const TextStyle(fontWeight: FontWeight.w600, fontSize: 15))),
          Expanded(
